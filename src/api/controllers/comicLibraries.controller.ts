...
export const comicLibraryController = {
    registerLibrary: async (c: Context) => {
        try {
            ...
            const parsed: ZodSafeParseResult<LibraryRegistrationInput> = ComicLibrarySchema.safeParse(libraryData);

            if (!parsed.success) {
                return c.json({ message: "Invalid library data", errors: parsed.error.flatten() }, 400);
            }

            // Use the service layer to handle library registration logic
            const newLibraryId = registerComicLibrary(parsed.data);

            return c.json({ message: `Library[${parsed.data.name}] registered successfully`, libraryId: newLibraryId }, 201);
        } catch (error) {
