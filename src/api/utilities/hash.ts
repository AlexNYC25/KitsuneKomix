...
export async function calculateImageHash(imagePath: string): Promise<string> {
  try {
    ...
    const supportedExtensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.bmp', '.tiff', '.tif', '.svg'];
    const fileExtension = imagePath.toLowerCase().split('.').pop();
    
    if (!fileExtension || !supportedExtensions.includes(`.${fileExtension}`)) {
      throw new Error(`Unsupported image format: ${fileExtension}. Supported formats: ${supportedExtensions.join(', ')}`);
    }
    ...
    if (error instanceof Deno.errors.NotFound) {
      ...
      throw new Error(`Failed to calculate image hash for ${imagePath}: ${errorMessage}`);
    }
  }
}
 ...
 */
export async function calculatePerceptualImageHash(imagePath: string): Promise<string> {
  try {
    ...
    console.warn("Perceptual hashing not fully implemented. Using content hash instead.");
    return await calculateImageHash(imagePath);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(`Failed to calculate perceptual hash for image ${imagePath}: ${errorMessage}`);
  }
}
 ...
 */
export async function calculateImageHashVariants(imagePath: string): Promise<ImageHashResult> {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(`Failed to calculate image hash variants for ${imagePath}: ${errorMessage}`);
  }
}
