...
import { comicStoryArcsTable, comicBookStoryArcsTable } from "../schema.ts";
	...
	try {
		...
		if (result.length === 0) {
			...
			if (existingStoryArc.length > 0) {
				console.log(`Comic story arc already exists with name: ${name}, returning existing ID: ${existingStoryArc[0].id}`);
				return existingStoryArc[0].id;
			}

			throw new Error(`Failed to insert comic story arc and could not find existing story arc. Name: ${name}`);
		}
...
export const linkStoryArcToComicBook = async (storyArcId: number, comicBookId: number): Promise<void> => {
	const { db, client } = getClient();
