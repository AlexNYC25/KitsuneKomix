...
import { comicLocationsTable, comicBookLocationsTable } from "../schema.ts";
	...
	try {
		...
		if (result.length === 0) {
			...
			if (existingLocation.length > 0) {
				console.log(`Comic location already exists at name: ${name}, returning existing ID: ${existingLocation[0].id}`);
				return existingLocation[0].id;
			}

			throw new Error(`Failed to insert comic location and could not find existing location. Name: ${name}`);
		}
...
export const linkLocationToComicBook = async (locationId: number, comicBookId: number): Promise<void> => {
	const { db, client } = getClient();
