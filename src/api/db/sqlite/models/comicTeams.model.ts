...
import { comicTeamsTable, comicBookTeamsTable } from "../schema.ts";
  ...
  try {
		...
		if (result.length === 0) {
			...
			if (existingTeam.length > 0) {
				console.log(`Comic team already exists with name: ${name}, returning existing ID: ${existingTeam[0].id}`);
				return existingTeam[0].id;
			}
			
			throw new Error(`Failed to insert comic team and could not find existing team. Name: ${name}`);
		}
...
export const linkTeamToComicBook = async (teamId: number, comicBookId: number): Promise<void> => {
	const { db, client } = getClient();
