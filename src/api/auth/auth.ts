...
import { ISSUER, AUDIENCE, ACCESS_TTL, REFRESH_TTL, ACCESS_KEY, REFRESH_KEY } from '../config/auth/jwtSetUp.ts';
 ...
 * @returns The decoded token payload.
 */
export async function verifyRefreshToken(token: string): Promise<RefreshClaims & jose.JWTPayload> {
 ...
 */
export async function generateTokenPair(userId: string, roles?: string[]): Promise<{
  accessToken: string;
  refreshToken: string;
