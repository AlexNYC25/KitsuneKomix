...
import { createTables } from "./initSchema.ts";
import { getDatabase } from "./sqlite.ts";

function dbTablesExist(): boolean {
    try {
        const db = getDatabase();
        const result = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='comic_libraries'").get();
...
export function initializeDatabase() {
    try {
        ...
        if (!tablesExist) {
            ...
            console.log("Database file and tables already exist, skipping initialization");
        }
