...
app.openapi(
  ...
  async (c) => {
    try {
      const allowedFilterValues = await compileEntireSeriesMetadataAndAdditionalSeriesInfo(c.get("userId")!);
...
app.openapi(
  ...
  async (c) => {
    ...
    try {
      ...
      const resultComics: ComicSeriesWithMetadata[] = hasNextPage
        ? comicSeries.slice(0, paginationData.pageSize)
        : comicSeries;
