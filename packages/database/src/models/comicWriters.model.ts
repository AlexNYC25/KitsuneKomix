...
import { comicBookWritersTable, comicWritersTable } from "../schemas/index.ts";
...
/**
 * Unlinks all writers from a comic book by removing all relationships in the junction table
 * @param comicBookId The ID of the comic book
 * @returns void
 */
export const unlinkWritersToComicBook = async (
  comicBookId: number,
): Promise<void> => {
  const { db, client } = getClient();
