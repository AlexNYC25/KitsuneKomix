...
import type { ComicGenre } from "../../shared/types/index.ts";
...
export const insertComicGenre = async (genreName: string): Promise<number> => {
  ...
  try {
    ...
    if (result.length > 1) {
      dbLogger.warn(
        `Multiple records inserted for comic genre. Returning the first one. Name: ${genreName}`,
      );
    }

    if (!result[0]) {
      throw new Error("No record returned when inserting comic genre.");
    }
...
/**
 * Unlinks all genres from a comic book by removing all relationships in the junction table
 * @param comicBookId The ID of the comic book
 * @returns void
 */
export const unlinkGenresToComicBook = async (
  comicBookId: number,
): Promise<void> => {
  const db = await getClient();
