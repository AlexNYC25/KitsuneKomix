...
import { comicBookPublishersTable, comicPublishersTable } from "../../schemas/index.ts";

import type { ComicPublisher } from "../../shared/types/index.ts";
...
export const insertComicPublisher = async (name: string): Promise<number> => {
  ...
  try {
    ...
    if (!result[0]) {
      throw new Error("No record returned when inserting comic publisher.");
    }

    return result[0].id;
  } catch (error) {
    dbLogger.error("Error inserting comic publisher:" + error);
...
export const unlinkPublishersToComicBook = async (
  ...
  try {
    ...
    dbLogger.error("Error unlinking publishers from comic book:" + error);
    throw error;
  }
};
...
export const getPublishersByComicBookId = async (
  ...
  try {
    ...
    dbLogger.error("Error fetching publishers for comic book:" + error);
    throw error;
  }
};
...
export const getPublisherIdsByFilter = async (
  ...
  try {
    ...
    dbLogger.error("Error fetching publisher IDs by filter:" + error);
    throw error;
  }
};
