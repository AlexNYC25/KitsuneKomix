...
import type { ComicBookThumbnail, BatchComicBookThumbnails } from "../../shared/types/index.ts";
...
export const insertComicBookThumbnail = async (
  ...
  try {
    ...
    if (hasComicBookIdColumn) {
      ...
      if (!result[0]) {
        throw new Error("No record returned when inserting comic book thumbnail.");
      }
...
export const insertCustomComicBookThumbnail = async (
  ...
  try {
    ...
    if (!result[0]) {
      throw new Error("No record returned when inserting custom comic book thumbnail.");
    }
...
export const getComicBooksThumbnailsByComicBookIdsBatch = async (
  ...
  try {
    ...
    result.forEach((thumbnail) => {
      if ( thumbnail && thumbnail.comicBookId) {
